...
public_router = APIRouter(
    prefix="/api/movies/{movie_id}/movie-times",
    tags=["public_movie_times"]
)


@router.get("", response_model=MovieTimeListResponse)
...
def create_movie_time(cinema_id: int, movie_id: int, payload: MovieTimeCreate, db: Session = Depends(get_db)):
    ...
    db.refresh(mt)
    return {
        "result": MovieTimeResponse.model_validate(mt),
        "errors": [],
